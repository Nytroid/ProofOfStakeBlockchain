...
class NodeAPI(FlaskView):
    ...
    def start(self, host, apiPort):
        NodeAPI.register(self.app, route_base='/')
        self.app.run(host=host, port=apiPort)

    def injectNode(self, injectedNode):   #Get the node properties from Node() Cant add as parameter bcs of some architectural issues
        global node
        node = injectedNode
